...
namespace Presestance.Repositories
{
    internal class GenericRepository<TEntity,TKey> : IGenericRepository<TEntity, TKey> where TEntity : BaseEntity<TKey>
    {
       ...
       => _storeDbContext.Set<TEntity>().Remove(entity);
        ...
        public async Task<IEnumerable<TEntity>> GetAllWithSpicificationAsync(Spicification<TEntity> spicification, bool TrackChanges = false)
        => await ApplySpecification(spicification).ToListAsync();


        public async Task<TEntity?> GetByIdWithSpicificationAsync(TKey key, Spicification<TEntity> spicification, bool TrackChanges = false)
        => await ApplySpecification(spicification).FirstOrDefaultAsync(x => x.Id.Equals(key));

        private IQueryable<TEntity> ApplySpecification(Spicification<TEntity> spicification)
        {
            var evaluator = new SpecificationEvaluator<TEntity>();
            return evaluator.GetQuery(_storeDbContext.Set<TEntity>().AsQueryable(), spicification);
        }
        #endregion
